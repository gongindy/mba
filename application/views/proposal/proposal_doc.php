...
<div class="topic"><strong>9.&nbsp;กลุ่มเป้าหมาย</strong> &emsp;<?php if($proposal[0]['pro_target'] == NULL) { echo '-'; } else { echo $proposal[0]['pro_target'];} ?></div>
 ...
 -
  ...
  <?php
  	$gantt =& get_instance();
	$gantt->load->library('gantt_chart');
	$gantt_chart = $gantt->gantt_chart->gen($plan);
	print_r($gantt_chart);
  ?>
  ...
  <!--
    <table style="width:100%;">
      ...
      <tbody>
      	...
      	<tr>
        <?php $date = $plan['plan_start']." - ".$plan['plan_end'];?>
        	<td style="line-height:25px; padding:12px;"><?php echo $i+1; ?>. <?php echo $plan['plan_content'];?></td>
            <td style="line-height:25px; padding:12px;" align="center"><?php echo $li_date->date_convert->rangedate_convert($date);?></td>
        </tr>
        <?php $i++;} ?>
      </tbody>
    </table>
